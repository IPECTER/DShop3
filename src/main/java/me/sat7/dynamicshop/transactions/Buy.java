...
public final class Buy {
    ...
    public static void buy(ItemTrade.CURRENCY currency, Player player, String shopName, String tradeIdx, ItemStack tempIS, double priceSum, boolean infiniteStock) {
        ...
        if (actualAmount <= 0) {
            String message = "";
            if (currency == ItemTrade.CURRENCY.VAULT) {
                message = DynamicShop.dsPrefix(player) + papi(player, t("MESSAGE.NOT_ENOUGH_MONEY").replace("{bal}", n(econ.getBalance(player))));
            } else if (currency == ItemTrade.CURRENCY.JOB_POINT) {
                message = DynamicShop.dsPrefix(player) + papi(player, t("MESSAGE.NOT_ENOUGH_POINT").replace("{bal}", n(JobsHook.getCurJobPoints(player))));
            }
        ...
        if (!infiniteStock && stockOld <= actualAmount) {
            player.sendMessage(DynamicShop.dsPrefix(player) + papi(player, t("MESSAGE.OUT_OF_STOCK")));
            data.get().set(tradeIdx + ".stock", stockOld);
            return;
        }
        ...
        if (currency == ItemTrade.CURRENCY.VAULT) {
            if (econ.getBalance(player) < priceSum) {
                player.sendMessage(DynamicShop.dsPrefix(player) + papi(player, t("MESSAGE.NOT_ENOUGH_MONEY").replace("{bal}", n(econ.getBalance(player)))));
                return;
            }
        ...
        while (leftAmount > 0) {
            ...
            if (leftOver.size() != 0) {
                player.sendMessage(DynamicShop.dsPrefix(player) + papi(player, t("MESSAGE.INVENTORY_FULL")));
        ...
        if (currency == ItemTrade.CURRENCY.VAULT) {
            message = DynamicShop.dsPrefix(player) + papi(player, t("MESSAGE.BUY_SUCCESS", !useLocalizedName)
                    .replace("{amount}", Integer.toString(actualAmount))
                    .replace("{price}", n(r.amount))
                    .replace("{bal}", n(econ.getBalance((player)))));
        } else if (currency == ItemTrade.CURRENCY.JOB_POINT) {
            message = DynamicShop.dsPrefix(player) + papi(player, t("MESSAGE.BUY_SUCCESS_JP", !useLocalizedName)
                    .replace("{amount}", Integer.toString(actualAmount))

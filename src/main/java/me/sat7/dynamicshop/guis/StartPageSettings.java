...
public final class StartPageSettings extends InGameUI
{
    ...
    public Inventory getGui(Player player, int slotIndex)
    {
        ...
        inventory = Bukkit.createInventory(player, 9, papi(player,t("START_PAGE.EDITOR_TITLE")));

        CreateCloseButton(player, CLOSE); // 닫기 버튼

        CreateButton(NAME, Material.BOOK, papi(player,t("START_PAGE.EDIT_NAME")), ""); // 이름 버튼
        CreateButton(LORE, Material.BOOK, papi(player,t("START_PAGE.EDIT_LORE")), ""); // 설명 버튼

        // 아이콘 버튼
        CreateButton(ICON, Material.getMaterial(StartPage.ccStartPage.get().getString("Buttons." + slotIndex + ".icon")), papi(player,t("START_PAGE.EDIT_ICON")), "");

        String cmdString = StartPage.ccStartPage.get().getString("Buttons." + slotIndex + ".action");
        CreateButton(CMD, Material.REDSTONE_TORCH, papi(player,t("START_PAGE.EDIT_ACTION")), cmdString == null || cmdString.isEmpty() ? null : "§7/" + cmdString); // 액션 버튼
        CreateButton(SHOP_SHORTCUT, Material.EMERALD, papi(player,t("START_PAGE.SHOP_SHORTCUT")), ""); // 상점 바로가기 생성 버튼
        CreateButton(DECO, Material.BLUE_STAINED_GLASS_PANE, papi(player,t("START_PAGE.CREATE_DECO")), ""); // 장식 버튼
        CreateButton(DELETE, Material.BONE, papi(player,t("START_PAGE.REMOVE")), papi(player,t("START_PAGE.REMOVE_LORE"))); // 삭제 버튼
    ...
    public void OnClickUpperInventory(InventoryClickEvent e)
    {
        ...
        {
            player.sendMessage(DynamicShop.dsPrefix(player) + papi(player,t("START_PAGE.ENTER_NAME")));
        ...
        {
            player.sendMessage(DynamicShop.dsPrefix(player) + papi(player,t("START_PAGE.ENTER_LORE")));
            ShopUtil.closeInventoryWithDelay(player);
        ...
        {
            player.sendMessage(DynamicShop.dsPrefix(player) + papi(player,t("START_PAGE.ENTER_ICON")));
            ShopUtil.closeInventoryWithDelay(player);
        ...
        {
            player.sendMessage(DynamicShop.dsPrefix(player) + papi(player,t("START_PAGE.ENTER_ACTION")));
            ShopUtil.closeInventoryWithDelay(player);
